    ...
    if (kind === 6) {
        ...
        if (mapped) {
            ...
            if (hex) {
                ...
                return isBlockedProxyAddress(`${high >> 8}.${high & 0xff}.${low >> 8}.${low & 0xff}`);
            }
        }
...
async function assertSafeProxyTarget(parsedUrl) {
    ...
    for (const address of addresses) {
        if (isBlockedProxyAddress(address)) {
            throw new ProxyTargetError(`Refusing to proxy request to blocked address (${address})`);
        }
    }
}
...
async function safeProxyFetch(initialUrl, init) {
